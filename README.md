# NEBULA NEXUS  
**EVE Online Sites Database** 🚀

Database referensi lengkap untuk **exploration sites** dan **combat anomalies** di game EVE Online.

[![GitHub Pages](https://img.shields.io/badge/GitHub_Pages-Deployed-brightgreen?logo=githubpages)](https://kingsyah.github.io/exploNexus/)  
![Status](https://img.shields.io/badge/Status-Work%20in%20Progress-yellow)

## Apa itu Nebula Nexus?

Nebula Nexus adalah **database visual** yang membantu pilot EVE Online menemukan dan memahami berbagai macam situs (sites) di New Eden, terutama:

- Exploration sites (Data, Relic, Gas, Wormhole)
- Combat anomalies
- Safe / high-value sites

Saat ini masih dalam tahap pengembangan awal — tabel masih kosong, tapi fitur filtering dan tampilan sudah siap.

## Fitur Utama

- 📊 **Counter real-time** Total Sites, Exploration, Combat, Safe Sites
- 🔍 **Filter cerdas** untuk jenis situs:
  - ✓ Safe Sites
  - ⛽ Gas Sites (Mykoserocin & Fullerite)
  - 🏺 Relic Sites
  - 💾 Data Sites
  - 🌀 Wormhole
- Faction filter: Angel Cartel, Blood Raiders, Guristas, Sansha, Serpentis, Rogue Drones
- 🛡️ **Highsec Only** toggle
- Tabel Exploration Sites: Site Name, Type, Safety, Region, Notes
- Tabel Combat Anomalies: Anomaly Name, Faction, Difficulty, Security, Escalation, Notes
- Desain clean & mobile-friendly

## Demo / Live Preview

👉 **[Buka Nebula Nexus](https://kingsyah.github.io/exploNexus/)**
